...
var Player = Class.extend({
	...
	jogar: function(coord){
		...
		if( result == Tabuleiro.TIRO_INVALIDO ){
			alert('VocÃª nÃ£o pode atirar em um lugar que vocÃª jÃ¡ atirou antes!');
		}
	},
...
var CPU = Player.extend({
	...
	start: function(){
		alert('Ã‰ a vez de '+this.nome+', aguarde');
	...
	logDeTiro: function(evt, coord, result, item){
		switch(result){
			case Tabuleiro.TIRO_AGUA:
				this.log(coord, this.jogadorAtual.nome + ' deu um tiro na Ã¡gua');
			break;
